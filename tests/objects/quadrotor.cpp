...
TEST(Quadrotor, Constructor) {
  ...
  const std::string cfg_path =
    std::string("/Users/yanxr/Desktop/mocap/src/quadrotor/configs/quadrotor.yaml");
...
TEST(Quadrotor, RunQuadCmdFeedThrough) {
  ...
  Command cmd(0.0, CommandType::SINGLE_ROTOR_THRUSTS);
  ...
  quad_state.p << 0.0, 0.0, 1.0;
  quad.reset(quad_state);
  cmd.type = CommandType::SINGLE_ROTOR_THRUSTS;
  cmd.t = 0.0;
...
TEST(Quadrotor, RunSimulatorBodyRate) {
  ...
  Command cmd(0.0, CommandType::RATES_THRUST);
