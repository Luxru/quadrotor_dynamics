...
bool QuadrotorDynamics::dState(const Ref<const Vector<QuadState::SIZE>> state,
  ...
  if (!state.segment<QS::NDYM>(0).allFinite())
    return false;
...
QuadrotorDynamics::DynamicsFunction
QuadrotorDynamics::getDynamicsFunction() const {
  return std::bind(
...
Vector<3> QuadrotorDynamics::clampTorques(const Vector<3>& torque) const {
  // ERROR: WRONG IMPLEMENTATION
  Vector<3> torque_max = J_ * omega_max_;
  return torque.cwiseMax(-torque_max).cwiseMin(torque_max);
}
...
Vector<4>
QuadrotorDynamics::motorThrustToOmega(const Vector<4> &thrusts) const {
...
Matrix<4, 4> QuadrotorDynamics::getAllocationMatrix() const {
  return (Matrix<4, 4>() << Vector<4>::Ones().transpose(), t_BM_.topRows<2>(),
    ...
    /*
    t_BM = [1, -1, -1, 1;
        -1, -1, 1, 1; * arm_l * sqrt(0.5)
        0, 0, 0, 0];
    * Allocation_ = [1, 1, 1, 1;  Total thrust
    *                (1, -1, -1, 1)*arm_l*sqrt(0.5); X-axis torque, roll
    *                (-1, -1, 1, 1)*arm_l*sqrt(0.5); Y-axis torque, pitch
    *                 (1, -1, 1, -1)*kappa_]; Z-axis torque, yaw

    * force_torques = [thrust1 + thrust2 + thrust3 + thrust4;
    *                  (thrust1 - thrust2 - thrust3 + thrust4)*arm_l*sqrt(0.5);
    *                  (-thrust1 - thrust2 + thrust3 + thrust4)*arm_l*sqrt(0.5);
    *                  (thrust1 - thrust2 + thrust3 - thrust4)*kappa_];

    * thrusts = [thrust1; thrust2; thrust3; thrust4] = Allocation^-1 * force_torques;

    Use torque to control the quadrotor contain some constraints.
    */
}
